...
//chart
$labels = ["< Rp 1.000.000", " Rp 1.000.000 > Rp 5.000.000", "Rp 5.000.000 > Rp 10.000.000", "> Rp 10.000.000"];

$arr_tampung1[] = $arr_tampung1["gaji"] = [];
$arr_tampung2[] = $arr_tampung2["gaji"] = [];
$arr_tampung3[] = $arr_tampung3["gaji"] = [];
$arr_tampung4[] = $arr_tampung4["gaji"] = [];


$query = mysqli_query($conn, "SELECT gaji, COUNT(id_pegawai) as jumlah_pegawai FROM gaji WHERE gaji<1000000");
while($row = mysqli_fetch_assoc($query)) {
    $arr_tampung1["jumlah_pegawai"] = $row["jumlah_pegawai"];
    $gaji[] = $row['gaji'];
}
$query = mysqli_query($conn, "SELECT gaji, COUNT(id_pegawai) as jumlah_pegawai FROM gaji WHERE gaji >= 1000000 AND gaji <= 5000000");
while($row = mysqli_fetch_assoc($query)) {
    $arr_tampung2["jumlah_pegawai"] = $row["jumlah_pegawai"];
    $gaji[] = $row['gaji'];
}
$query = mysqli_query($conn, "SELECT gaji, COUNT(id_pegawai) as jumlah_pegawai FROM gaji WHERE gaji >= 5000000 AND gaji <= 10000000");
while($row = mysqli_fetch_assoc($query)) {
    $arr_tampung3["jumlah_pegawai"] = $row["jumlah_pegawai"];
    $gaji[] = $row['gaji'];
}
$query = mysqli_query($conn, "SELECT gaji, COUNT(id_pegawai) as jumlah_pegawai FROM gaji WHERE gaji > 10000000");
while($row = mysqli_fetch_assoc($query)) {
    $arr_tampung4["jumlah_pegawai"] = $row["jumlah_pegawai"];
    $gaji[] = $row['gaji'];
}
    //$arr_tampung["jumlah_pegawai"][] = $row["jumlah_pegawai"];
    // var_dump($arr_tampung["jumlah_pegawai"]);
        ...
        <div class="main-content">
            <div class="main-content-inner">
                ...
                <div class="page-header">
                    <h1>Rekapitulasi<small> <i class="ace-icon fa fa-angle-double-right"></i> Grafik Gaji
                            &nbsp;
                        </small>
                    </h1>
                </div>
                ...
                <div style="padding-left: 30px; padding-right: 30px">
                    <div class="col-8 offset-2">
                        <div class="card">
                            <div class="card-body">
                                <h2><b>Grafik Gaji Pegawai BUMN 2022-2023</b></h2>
                                <hr>
                                <center>
        ...
        data: {
            ...
            datasets: [{
                ...
                data: [<?php echo json_encode($arr_tampung1["jumlah_pegawai"]);  ?>,
                    <?= json_encode($arr_tampung2["jumlah_pegawai"]) ?>,
                    <?= json_encode($arr_tampung3["jumlah_pegawai"]) ?>,
                    <?= json_encode($arr_tampung4["jumlah_pegawai"]) ?>
                ],
                backgroundColor: [
                    'rgba(255, 99, 132, 0.2)',
                    'rgba(255, 159, 64, 0.2)',
